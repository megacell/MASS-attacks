...
class OptimalAttackSolver:
    ...
    def status(self, iter, last_step):
        ...
        if iter > 0:
            ...
            main_obj = int(obj + reg + self.omega*thru)
            if self.init_obj is None: self.init_obj = main_obj
            msg = template.format(iter, last_step, \
                            int(obj), int(self.initial), int(100.0 * obj / self.initial), \
                            main_obj, int(100.0 * main_obj / self.init_obj), \
                            100.0 * (self.last - main_obj) / main_obj, \
                            int(thru), \
                            reg, \
